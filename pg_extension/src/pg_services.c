/*
 * =============================================================================
 * pg_services.c — Camada de SERVICES da extensão PostgreSQL
 * =============================================================================
 *
 * Lógica de negócio sobre a SPI (Server Programming Interface):
 *   - Helpers de banco: INSERT/UPDATE/DELETE de nós, revisões, paths intermediários
 *   - Flatten/insert recursivo (JSON → nodes) com inline optimization + arrays
 *   - Deep merge (update_json)
 *   - Reconstrução de árvore (extract_json)
 *   - Query com filtros/ordenação/paginação (incl. wildcards com captura)
 *   - Módulo CSV (export/import)
 *
 * Os SERVICES recebem strings C e retornam strings palloc'd em fn_mcxt
 * (o caller é dono da memória). Cada service gerencia seu próprio
 * SPI_connect/SPI_finish.
 *
 * Dependências:
 *   - src/pg_utils.h  (utils: paths, wildcards, uuid, valores)
 ...
 * RECONSTRUÇÃO DE ÁRVORE (extract_json)
 * ===========================================================================
 * Constrói a árvore yyjson a partir das linhas ordenadas por path.
 * Retorna o root (ou NULL se não houver linhas) e define *out_rows_count.
 */
static yyjson_mut_val *build_tree_from_rows(yyjson_mut_doc *mut_doc,
                                            int nrows, TupleDesc tupdesc,
                                            HeapTuple *vals, int *out_rows_count)
{
  *out_rows_count = 0;
  if (nrows <= 0 || !vals || !tupdesc)
    return NULL;
 ...
 * ===========================================================================
 */
static void query_wildcard(const char *pattern, const char *query_str,
...
/* ===========================================================================
 * SERVICE: SET_JSON (JSON -> Nodes, replace completo)
 * ===========================================================================
 */
char *service_set_json(const char *doc_id, const char *json_str, MemoryContext fn_mcxt)
{
  ...
  if (yyjson_is_null(root))
  {
    ...
    SPI_finish();
    return MemoryContextStrdup(fn_mcxt, revision);
  }
...
/* ===========================================================================
 * SERVICE: UPDATE_JSON (JSON -> Nodes, deep merge)
 * ===========================================================================
 */
char *service_update_json(const char *doc_id, const char *json_str, MemoryContext fn_mcxt)
{
  ...
  if (yyjson_is_null(update_root))
  {
    ...
    SPI_freeplan(upd_empty);
    SPI_finish();
    return MemoryContextStrdup(fn_mcxt, rev);
  }
...
/* ===========================================================================
 * SERVICE: EXTRACT_JSON (Nodes -> JSON)
 * ===========================================================================
 */
char *service_extract_json(const char *input, MemoryContext fn_mcxt)
{
  ...
  int nrows = (int)SPI_processed;
  TupleDesc tupdesc = SPI_tuptable ? SPI_tuptable->tupdesc : NULL;
  HeapTuple *vals = SPI_tuptable ? SPI_tuptable->vals : NULL;

  yyjson_mut_doc *mut_doc = yyjson_mut_doc_new(NULL);
  int rows_count = 0;
  yyjson_mut_val *root = build_tree_from_rows(mut_doc, nrows, tupdesc, vals,
                                              &rows_count);

  SPI_freeplan(stmt);
  SPI_finish();

  if (!root || rows_count == 0)
  {
  ...
  char *result = MemoryContextStrdup(fn_mcxt, json_out);
  free(json_out);
  return result;
}

/* ===========================================================================
 * SERVICE: QUERY_JSON (filtros/ordenação/paginação; suporta wildcard)
 * ===========================================================================
 */
char *service_query_json(const char *prefix_input, const char *query_str, MemoryContext fn_mcxt)
{
  ...
  if (path_has_wildcard(prefix_input))
  {
    char *out = NULL;
    query_wildcard(prefix_input, query_str, &out, fn_mcxt);
    SPI_finish();
    return out;
  }
  ...
  if (json_out)
  {
    result = MemoryContextStrdup(fn_mcxt, json_out);
    free(json_out);
  }
...
/* Constrói a árvore yyjson a partir das linhas CSV (ordenadas por path). */
...
/* ===========================================================================
 * SERVICE: EXPORT_CSV (Nodes -> CSV)
 * ===========================================================================
 */
char *service_export_csv(const char *input, MemoryContext fn_mcxt)
{
  if (!input || input[0] == '\0')
    return MemoryContextStrdup(fn_mcxt, CSV_HEADER);
  ...
  char *result = MemoryContextStrdup(fn_mcxt, out->data);
  SPI_freeplan(stmt);
  SPI_finish();
...
/* ===========================================================================
 * SERVICE: IMPORT_CSV (CSV -> Nodes)
 * ===========================================================================
 */
char *service_import_csv(const char *prefix_input, const char *csv_text, MemoryContext fn_mcxt)
{
  size_t max_inline_size = DEFAULT_MAX_INLINE_SIZE;
